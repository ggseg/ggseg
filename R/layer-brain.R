...
LayerBrain <- ggproto(
  ...
  setup_layer = function(self, data, plot) {
    ...
    if (class(dt)[1] != "waiver") {
      ...
      merge_errs <- sapply(data$geometry, function(x) {
        ifelse(length(!is.na(x)) > 0, TRUE, FALSE)
      })
    ...
    if (
      (isTRUE(self$inherit.aes) &&
        is.null(self$computed_mapping$hemi) &&
        is.null(plot$computed_mapping$hemi)) ||
    ...
    if (
      (isTRUE(self$inherit.aes) &&
        is.null(self$computed_mapping$side) &&
        is.null(plot$computed_mapping$side)) ||
    ...
    if (
      (isTRUE(self$inherit.aes) &&
        is.null(self$computed_mapping$type) &&
        is.null(plot$computed_mapping$type)) ||
    ...
    if (
      (isTRUE(self$inherit.aes) &&
        is.null(self$computed_mapping$fill) &&
        is.null(plot$computed_mapping$fill)) ||
