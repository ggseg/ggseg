...
```{r plot.area}
ggbrain(plot.area="transversetemporal") +
  theme_void()
```
...
```{r plot.area2}
ggbrain(plot.area=c("transversetemporal", "insula","precentral","superiorparietal")) +
  theme_void()
```

You can also choose to only view one of the hespheres by adding the `hemisphere` option
```{r hemisphere}
ggbrain(hemisphere="rh") +
  theme_void()
```
...
```{r datasupp}
someData = list(area=c("transversetemporal", "insula","precentral","superiorparietal"), 
