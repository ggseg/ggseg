...
The function `read_freesurfer_stats` is made to easily read in raw stats tables from each individual, without needing to go through Freesurfers internal converters.
...
data %>% 
  ...
  ggseg(atlas = dkt, mapping = aes(fill = ThickAvg))
```
...
The error thrown is because the Freesurfer tables also include measures of total volume/area/thickness, estimated intracranial volume etc, which will not merge into the atlas, and the internal ggseg atlas-merging function throws a warning. 
